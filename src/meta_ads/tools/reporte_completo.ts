...
export async function generarReporteCompletoCampana(
  campaignId: string,
  opts: { datePreset?: string; timeRange?: { since: string; until: string }; incluirAnuncios?: boolean } = {},
): Promise<string> {
  initApi();
  const incluir_anuncios = opts.incluirAnuncios ?? true;
  const rangoParam: Record<string, any> = opts.timeRange
    ? { time_range: JSON.stringify(opts.timeRange) }
    : { date_preset: opts.datePreset ?? 'last_30d' };
  const periodoLabel = opts.timeRange ? `${opts.timeRange.since} → ${opts.timeRange.until}` : (opts.datePreset ?? 'last_30d');

  const camp = new Campaign(campaignId);
  await camp.read([
  ...
  const totalCursor = await camp.getInsights(INSIGHT_FIELDS, { ...rangoParam });
  const total = cursorToArray(totalCursor)[0];

  const adsetInsCursor = await camp.getInsights([...INSIGHT_FIELDS, 'adset_id'], { ...rangoParam, level: 'adset', limit: 500 });
  const metricasAdset = indexarInsights(adsetInsCursor, 'adset_id');

  let metricasAd = new Map<string, any>();
  if (incluir_anuncios) {
    const adInsCursor = await camp.getInsights([...INSIGHT_FIELDS, 'ad_id', 'adset_id'], { ...rangoParam, level: 'ad', limit: 500 });
  ...
  const insightsDesglose = (bk: string) =>
    camp.getInsights([...INSIGHT_FIELDS, 'adset_id'], { ...rangoParam, level: 'adset', breakdowns: [bk], limit: 500 });
...
export function registrarHerramientasReporteCompleto(server: McpServer) {
  server.registerTool(
    ...
    async ({ campaign_id, rango_fechas, incluir_anuncios }) => {
      ...
      try {
        const texto = await generarReporteCompletoCampana(campaign_id, { datePreset: rango_fechas, incluirAnuncios: incluir_anuncios });
