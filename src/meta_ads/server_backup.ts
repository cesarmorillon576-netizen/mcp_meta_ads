...
server.registerTool(
  'obtener_config_mensajeria_adsset',
  {
    description: 'Obtiene configuración de mensajería para un Ad Set (debug) leyendo fields desde la API/SDK.',
    inputSchema: {
      adsset_id: z.string().describe('ID del conjunto de anuncios'),
      debug: z.boolean().default(false).describe('Si true, devuelve el raw JSON con todos los campos leídos'),
    },
  },
  async ({ adsset_id, debug }) => {
    ...
    try {
      ...
      // Fase 1: campos siempre válidos en AdSet
      const baseFields = [
      ...
      // Fase 2: campos opcionales confirmados como válidos en este tipo de cuenta
      ...
      // Probar cada campo candidato individualmente para no abortar todo
      ...
      const lines = [
        ...
        `- targeting (resumen): ${d.targeting ? JSON.stringify(d.targeting).slice(0, 200) : 'N/A'}`,
      ];
